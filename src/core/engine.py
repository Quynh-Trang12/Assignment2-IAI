...
from src.core.models import SearchState  # type: ignore
...
class SearchEngine:
    ...
    # ---------------------------------------------------------------------------
    def solve(self, search_method: str) -> Any:
        """
        ...
        Args:
            search_method (str): A string indicating the algorithm (e.g., 'dfs', 'topk-5').

        Returns:
            Any: A single payload tuple for 2A algorithms, or a List of tuples for Top-K.
        """
        # If origin or destination isn't in the graph, don't even try finding it.
        if self.graph.origin not in self.graph.node_coordinates:
            print(f"Error: Origin {self.graph.origin} not found in graph coordinates.")
            return None

        normalized_method = search_method.lower()

        # Assignment 2B: Top-K Intercept
        if normalized_method.startswith("topk"):
            try:
                k_val = int(normalized_method.split("-")[1])
            except (IndexError, ValueError):
                k_val = 5
            return self._execute_yens_k_shortest_paths(k_val)

        # Assignment 2A: Legacy Routing
        ...
        while stack:
            ...
            for neighbor_identifier, edge_weight in neighbors:
                if neighbor_identifier not in visited_nodes:
                    ...
                    new_state = self._create_search_state(
                        neighbor_identifier, current_state, new_cumulative_cost, "dfs"
                    )
        ...
        while queue:
            ...
            for neighbor_identifier, edge_weight in neighbors:
                if neighbor_identifier not in visited_nodes:
                    ...
                    new_cumulative_cost = current_state.g + edge_weight
                    new_state = self._create_search_state(
                        neighbor_identifier, current_state, new_cumulative_cost, "bfs"
                    )
    ...
    def _execute_priority_search(
        ...
        """
        start_state = self._create_search_state(
            self.graph.origin, None, 0.0, search_method
        )
        ...
        while open_priority_queue:
            ...
            for neighbor_identifier, edge_weight in self.graph.get_neighbors(
                current_state.node_id
            ):
    ...
    # ---------------------------------------------------------------------------
    def _execute_iterative_deepening_a_star(
        self,
    ) -> Optional[Tuple[int, int, List[int]]]:
        """
        ...
        while True:
            ...
            start_state = self._create_search_state(
                self.graph.origin, None, 0.0, "cus2"
            )

            search_result = self._iterative_deepening_recursive(
                start_state, current_threshold, [self.graph.origin]
            )
            ...
            if isinstance(search_result, SearchState):
                return (
                    ...
                    self.total_nodes_created,
                    self._reconstruct_path(search_result),
                )
    ...
    def _iterative_deepening_recursive(
        ...
        for neighbor_identifier, edge_weight in neighbors:
            ...
            if neighbor_identifier not in current_path_identifiers:
                new_cumulative_cost = current_state.g + edge_weight
                new_child_state = self._create_search_state(
                    neighbor_identifier, current_state, new_cumulative_cost, "cus2"
                )
        ...
        for child_state in child_states:
            ...
            recursive_result = self._iterative_deepening_recursive(
                child_state, current_threshold, extended_path
            )
    ...
    # ---------------------------------------------------------------------------
    # Assignment 2B: Top-K Shortest Paths (Yen's Algorithm)
    # ---------------------------------------------------------------------------
    def _execute_yens_k_shortest_paths(
        self, k: int
    ) -> List[Tuple[int, int, List[int]]]:
        """
        Executes Yen's Algorithm to find the Top-K shortest loopless paths.

        Architectural Note:
        Yen's Algorithm acts as a meta-algorithm. It leverages A* Search to find the
        baseline optimal path, then systematically suppresses nodes and edges to force
        the discovery of the next-best alternative routes without creating cycles.

        Args:
            k (int): The maximum number of distinct paths to discover.

        Returns:
            List[Tuple[int, int, List[int]]]: A list of successful routing payloads.
        """
        first_result = self._execute_bounded_a_star(self.graph.origin, set(), set())
        if not first_result:
            return []

        _, _, first_path = first_result

        def _compute_path_cost(path_sequence: List[int]) -> float:
            cost = 0.0
            for i in range(len(path_sequence) - 1):
                cost += self.graph.adjacency_list[path_sequence[i]][
                    path_sequence[i + 1]
                ]
            return cost
        ...
        for k_index in range(1, k):
            for i in range(len(A_paths[k_index - 1]) - 1):
                spur_node_identifier = A_paths[k_index - 1][i]
                root_sequence = A_paths[k_index - 1][: i + 1]

                suppressed_edges: Set[Tuple[int, int]] = set()
                suppressed_nodes: Set[int] = set()

                for existing_path in A_paths:
                    if (
                        len(existing_path) > i
                        and existing_path[: i + 1] == root_sequence
                    ):
                        suppressed_edges.add((existing_path[i], existing_path[i + 1]))

                for root_node in root_sequence[:-1]:
                    suppressed_nodes.add(root_node)

                spur_result = self._execute_bounded_a_star(
                    spur_node_identifier, suppressed_edges, suppressed_nodes
                )

                if spur_result:
                    _, _, spur_sequence = spur_result
            ...
            while B_candidates:
                ...
                if optimal_candidate_path not in A_paths:
                    A_paths.append(optimal_candidate_path)
                    paths_nodes_explored.append(
                        self.total_nodes_created
                    )  # Snapshot memory!
                    is_new_path_found = True
                    break

            if not is_new_path_found:
                break

        formatted_payloads = []
        for idx, path in enumerate(A_paths):
            goal_identifier = path[-1]
            formatted_payloads.append(
                (goal_identifier, paths_nodes_explored[idx], path)
            )

        return formatted_payloads

    def _execute_bounded_a_star(
        self,
        start_node_identifier: int,
        suppressed_edges: Set[Tuple[int, int]],
        suppressed_nodes: Set[int],
    ) -> Optional[Tuple[int, int, List[int]]]:
        """
        A highly specialized A* variant utilized exclusively by Yen's Algorithm.

        Architectural Note:
        This respects all standard A* kinematics and tie-breaking protocols, but injects
        dynamic network suppression. This allows the orchestrator to temporarily "block"
        roads and intersections without permanently mutating the master Graph object.
        """
        start_state = self._create_search_state(start_node_identifier, None, 0.0, "as")
        open_priority_queue: List[SearchState] = [start_state]
        closed_set: Set[int] = set()
        ...
        while open_priority_queue:
            ...
            for neighbor_identifier, edge_weight in self.graph.get_neighbors(
                ...
                # Dynamic Suppression Injection: Ignore blocked roads and nodes
                if neighbor_identifier in suppressed_nodes:
                    continue
                if (current_state.node_id, neighbor_identifier) in suppressed_edges:
                    continue

                if neighbor_identifier not in closed_set:
                    new_cumulative_cost = current_state.g + edge_weight
                    new_state = self._create_search_state(
                        neighbor_identifier, current_state, new_cumulative_cost, "as"
                    )
                    heapq.heappush(open_priority_queue, new_state)
