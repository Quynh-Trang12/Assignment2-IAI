    ...
    def load_from_file(self, filepath: str) -> None:
        ...
        for line in sanitized_lines:
            ...
            if current_section == "Nodes":
                ...
                # Expected Schema: "5; 4"
                self.destinations = [
                    int(destination_id) for destination_id in line.split(";")
                ]
    ...
    def heuristic(self, node_identifier: int) -> float:
        ...
        for destination_identifier in self.destinations:
            ...
            euclidean_distance = math.sqrt(
                (current_x - destination_x) ** 2 + (current_y - destination_y) ** 2
            )

            # 1. Scale to match the map_generator.py physical dimensions
            scaled_distance = euclidean_distance * 100.0

            # 2. Contextualize the Unit (Distance vs. Time)
            if self.is_time_based:
                # Admissible Time Heuristic: Distance / Absolute Max Speed (60 km/h)
                # Guarantees the heuristic never overestimates the remaining travel time.
                h_value = scaled_distance / 60.0
            else:
                h_value = scaled_distance

            if h_value < minimum_heuristic_distance:
                minimum_heuristic_distance = h_value
